...
void loop() {
  ...
  // ตรวจสอบสถานะของ pumzone1 และ pumzone2 และทำการควบคุม relay
  controlRelay(relayPin, pumzone1);
  controlRelay(relayPin2, pumzone2);

  // ตรวจสอบสถานะของ autopum และทำการควบคุม relay ตามค่าความชื้น
  if(autopum == true){
  ...
  if (Firebase.isTokenExpired()) {
    ...
    if(autopum == true){
    digitalWrite(ledPin, LOW); // ให้ไฟ LED ติด
  }
  else {
...
void controlRelay(int relayPin, bool status) {
  if (status == true) {
