...
void loop() {
 digitalWrite(ledPin, HIGH);
  ...
  Serial.printf("วันเวลาปัจจุบัน: %04d-%02d-%02d %02d:%02d\n", 
        ...
        if (timesArray.get(timeData, i)) {
          if (timeData.typeNum == FirebaseJson::JSON_OBJECT) {
            ...
            if (currentHour == scheduleHour && currentMinute == scheduleMinute) {
              Serial.println("ถึงเวลารดปุ๋ยแล้ว!");
              waterPlants(duration, now);
            }
          } else {
            Serial.printf("ประเภทข้อมูลไม่ถูกต้องสำหรับเวลา %d\n", i);
          }
        } else {
          Serial.printf("ไม่สามารถรับข้อมูลสำหรับเวลา %d\n", i);
        }
      }
    } else {
      Serial.println("วันนี้ไม่มีการตั้งเวลารดปุ๋ย");
    }
  } else {
    Serial.printf("ไม่พบกำหนดการสำหรับวันนี้ (%s)\n", currentDate.c_str());
  }
